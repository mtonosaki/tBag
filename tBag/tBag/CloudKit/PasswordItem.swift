...
extension Item {
    // Note: PasswordFilter and PasswordAttributeKeys are now defined in Item.swift
    ...
    private func containsTag(_ tag: String, rsa: Rsa) -> Bool {
        guard let sealedString = self.attributes[GeneralAttributeKeys.tags.rawValue] else { return false }
        do {
            let tagsString = try CryptoService.shared.open(sealedString: sealedString, myRsa: rsa)
...
struct ItemBuilder {
    static func createNewPasswordItem(ownerAccountId: String, myRsa: Rsa) -> Item {
        ...
        if let sealed = try? CryptoService.shared.seal(plainText: tags, recipientPublicKey: myRsa.getMyPublicKey(), salt: salt) {
            newItem.attributes[Item.GeneralAttributeKeys.tags.rawValue] = sealed
        }
        return newItem
    }
}
